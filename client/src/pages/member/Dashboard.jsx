...
const QUICK_ACTIONS = [
  { label: 'Add Funds',    to: '/dashboard/topup',     icon: Wallet,          color: '#02d8dc', bg: 'rgba(2,216,220,0.08)', desc: 'Deposit USDT'    },
  { label: 'Invest Now',   to: '/dashboard/trade',     icon: TrendingUp,      color: '#3b82f6', bg: 'rgba(59,130,246,0.08)', desc: 'Start earning'   },
  { label: 'Withdraw',     to: '/dashboard/withdraw',  icon: ArrowUpFromLine, color: '#f97316', bg: 'rgba(249,115,22,0.08)', desc: 'Cash out funds'  },
  { label: 'My Network',   to: '/dashboard/genealogy', icon: Users,           color: '#7c3aed', bg: 'rgba(124,58,237,0.08)', desc: 'View your team'  },
]
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1.75rem' }}>
        ...
        <div style={{ position: 'relative', zIndex: 1, display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '1.5rem' }}>
          <div>
            ...
            <h1 style={{
              ...
              Welcome back, <span style={{ background: 'linear-gradient(135deg, #02d8dc, #3b82f6)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text' }}>{user?.name?.split(' ')[0] || 'Member'}</span>
            </h1>
          ...
          <div style={{ display: 'flex', gap: '0.75rem', alignItems: 'center' }}>
            ...
            <Link to="/dashboard/trade" style={{
              ...
              boxShadow: '0 4px 16px rgba(2,216,220,0.3)',
              transition: 'all 0.2s',
            }}>
              <Zap size={16} />
              Start Investing
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '1.25rem' }} id="dash-income">
        <StatCard label="Total ROI"            value={fmt(stats?.total_roi)}            icon={TrendingUp} color="purple" sub="Compounding"  />
        <StatCard label="Sponsor Income"       value={fmt(stats?.total_sponsor_income)} icon={Users}      color="green"  sub="L1+L2+L3"    />
        <StatCard label="Today's ROI"          value={fmt(stats?.today_roi)}            icon={Activity}   color="blue"   sub="Today"       />
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '1rem' }} id="dash-secondary">
        {[
          { label: "Today's ROI",       val: fmt(stats?.today_roi),               color: '#02d8dc', bg: 'rgba(2,216,220,0.08)', icon: Zap           },
          { label: "Today's Business",  val: fmt(stats?.today_business),          color: '#3b82f6', bg: 'rgba(59,130,246,0.08)', icon: BarChart3      },
          { label: 'Active Members',    val: fmtN(stats?.active_team),            color: '#10b981', bg: 'rgba(16,185,129,0.08)', icon: ShieldCheck   },
          { label: 'Total Team',        val: fmtN(stats?.team_total),             color: '#7c3aed', bg: 'rgba(124,58,237,0.08)', icon: Network       },
        ].map((s, i) => (
          <div key={i} style={{
          ...
          >
            ...
            <div>
              ...
              <p style={{ margin: 0, fontSize: '1.0625rem', fontWeight: 900, color: '#ffffff', fontFamily: 'Outfit, sans-serif', letterSpacing: '-0.01em' }}>{s.val}</p>
            </div>
          </div>
        ))}
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1.5fr 1fr', gap: '1.5rem' }} id="dash-main-row">
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
          ...
          <Panel style={{ padding: 0, overflow: 'hidden' }}>
            <div style={{ padding: '1.25rem 1.5rem', borderBottom: '1px solid var(--border-subtle)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <div>
                <p style={{ margin: 0, fontWeight: 800, fontSize: '0.9375rem', color: '#ffffff', fontFamily: 'Outfit, sans-serif' }}>BTC/USD Live Chart</p>
              ...
              <div style={{ display: 'flex', gap: '0.375rem' }}>
                {['1h', '4h', '1d'].map(t => (
                  <span key={t} style={{
                    ...
                    background: t === '1h' ? '#02d8dc' : 'rgba(255,255,255,0.04)',
                    color: t === '1h' ? '#080f1e' : '#cbd5e1',
                    borderRadius: 8, cursor: 'pointer',
                    border: t === '1h' ? 'none' : '1px solid rgba(255,255,255,0.08)',
                  }}>{t}</span>
                ))}
              </div>
            </div>
            ...
            {QUICK_ACTIONS.map(({ label, to, icon: Icon, color, bg, desc }) => (
              ...
              >
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <p style={{ margin: 0, fontWeight: 800, fontSize: '0.9375rem', color: '#ffffff', fontFamily: 'Outfit, sans-serif' }}>{label}</p>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
          ...
          <div style={{
            padding: '1.75rem',
            background: 'linear-gradient(135deg, #040d1a 0%, #0a1628 100%)',
            border: '1px solid rgba(2,216,220,0.25)',
            borderRadius: 20,
            color: '#fff',
            position: 'relative',
            overflow: 'hidden',
            boxShadow: '0 8px 32px rgba(2,216,220,0.06)',
          }}>
            <div style={{ position: 'absolute', top: '-20%', right: '-10%', width: 160, height: 160, background: 'radial-gradient(circle, rgba(2,216,220,0.08) 0%, transparent 70%)', borderRadius: '50%' }} />

            <div style={{ position: 'relative', zIndex: 1 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.625rem', marginBottom: '1.125rem' }}>
                <div style={{ width: 34, height: 34, borderRadius: 10, background: 'rgba(2,216,220,0.1)', color: '#02d8dc', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <Share2 size={16} />
                </div>
                <div>
                  <p style={{ margin: 0, fontWeight: 800, fontSize: '0.9375rem', fontFamily: 'Outfit, sans-serif', color: '#ffffff' }}>Invite & Earn</p>
              ...
              <div style={{ position: 'relative', marginBottom: '1.125rem' }}>
                ...
                <button onClick={copyReferral} style={{
                  ...
                  color: copied ? '#10b981' : 'rgba(2,216,220,0.7)', transition: 'color 0.2s',
                }}>
              ...
              <button
                ...
                style={{
                  ...
                  border: '1px solid rgba(2,216,220,0.25)',
                  borderRadius: 10, color: '#02d8dc',
                  fontSize: '0.8125rem', fontWeight: 700,
                  cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.5rem',
                  transition: 'background 0.2s',
                }}
                onMouseEnter={e => { e.currentTarget.style.background = 'rgba(2,216,220,0.18)' }}
                onMouseLeave={e => { e.currentTarget.style.background = 'rgba(2,216,220,0.1)' }}
              >
          ...
          <Panel>
            <div style={{ marginBottom: '1.25rem' }}>
              <p style={{ margin: 0, fontWeight: 800, fontSize: '0.9375rem', color: '#ffffff', fontFamily: 'Outfit, sans-serif' }}>Platform Status</p>
              <p style={{ margin: '0.25rem 0 0', fontSize: '0.75rem', color: '#94a3b8', fontWeight: 500 }}>Live performance indicators</p>
            </div>

            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
              {[
                { label: 'Daily Return Rate', val: '2.0% Compounding',  color: '#02d8dc' },
                { label: 'Min. Package',      val: '$150',               color: '#3b82f6' },
                { label: 'Income Lock',       val: '60 Days',            color: '#7c3aed' },
                { label: 'Sponsor L1 / L2',   val: 'Fixed $ per pkg',    color: '#f59e0b' },
              ].map((item, i) => (
                <div key={i} style={{
                  ...
                  <span style={{ fontSize: '0.8125rem', color: 'var(--text-muted)', fontWeight: 600 }}>{item.label}</span>
            ...
            <Link to="/dashboard/trade" style={{
              ...
              color: '#fff', borderRadius: 12, textDecoration: 'none',
              fontWeight: 700, fontSize: '0.875rem',
              boxShadow: '0 4px 14px rgba(2,216,220,0.25)',
            }}>
              <Zap size={16} />
              Start Investing Now
            </Link>

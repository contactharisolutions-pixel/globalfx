...
const COLUMNS = [
  ...
  { key: 'amount',     label: 'Amount', render: (v) => <span style={{ fontWeight: 900, color: 'var(--text-main)', fontFamily: 'Outfit, sans-serif' }}>${(+v).toLocaleString()}</span> },
  { key: 'tx_hash',   label: 'TX ID',  render: (v) => v ? (
    <span style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: '0.75rem', color: 'var(--primary)', display: 'flex', alignItems: 'center', gap: '0.375rem' }}>
      {v.slice(0, 10)}…{v.slice(-8)}
      <ExternalLink size={11} style={{ opacity: 0.6 }} />
    </span>
  ) : '—' },
  { key: 'status',     label: 'Status', render: (v) => <Badge status={v} /> },
  { key: 'note',       label: 'Note',   render: (v) => <span style={{ fontSize: '0.8125rem', color: 'var(--text-faint)', fontStyle: v ? 'normal' : 'italic' }}>{v || 'Pending review'}</span> },
]
        ...
        <div className="scale-in">
          {data.length > 0 && (
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.625rem', marginBottom: '1rem' }}>
              <ArrowDownCircle size={16} style={{ color: 'var(--primary)' }} />

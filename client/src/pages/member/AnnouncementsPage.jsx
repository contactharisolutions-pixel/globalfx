...
const PRIORITY_CONFIG = {
  urgent: { icon: Zap,           color: '#ef4444', bg: '#fef2f2',  border: 'rgba(239,68,68,0.25)',   label: 'Urgent'    },
  high:   { icon: AlertTriangle, color: '#f97316', bg: '#fff7ed',  border: 'rgba(249,115,22,0.25)',  label: 'Important' },
  normal: { icon: Info,          color: '#0d9488', bg: '#f0fdfa',  border: 'rgba(13,148,136,0.25)', label: 'Notice'    },
}
...
function AnnouncementCard({ ann }) {
  ...
  return (
    <Panel style={{ padding: 0, overflow: 'hidden', border: `1px solid ${open ? cfg.color : 'var(--border-subtle)'}`, background: open ? cfg.bg : '#ffffff', transition: 'var(--transition-normal)' }}>
      <button
      ...
      >
        <div style={{ width: 32, height: 32, borderRadius: 8, background: 'rgba(0,0,0,0.03)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, marginTop: '2px' }}>
          <Icon size={16} style={{ color: cfg.color }} />
        </div>
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', flexWrap: 'wrap' }}>
            <span style={{ fontSize: '0.9375rem', fontWeight: 800, color: '#0f172a' }}>{ann.title}</span>
            <span style={{
      ...
      {open && (
        <div className="scale-in" style={{ padding: '0 1.25rem 1.5rem 4rem' }}>
          <p style={{ fontSize: '0.875rem', color: '#475569', lineHeight: 1.7, whiteSpace: 'pre-wrap' }}>{ann.body}</p>
        </div>
      )}
    </Panel>
  )
                ...
                {urgent.length > 0 && (
                   <div style={{ display: 'flex', alignItems: 'center', gap: '0.625rem', marginBottom: '0.5rem' }}>
                      <Bell size={16} style={{ color: '#94a3b8' }} />
                       <p style={{ fontSize: '0.75rem', fontWeight: 900, color: '#94a3b8', textTransform: 'uppercase', letterSpacing: '0.08em' }}>All Updates</p>
                   </div>
                )}

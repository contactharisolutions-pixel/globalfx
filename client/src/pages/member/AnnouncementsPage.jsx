...
const PRIORITY_CONFIG = {
  urgent: { icon: Zap,           color: '#ef4444', bg: 'rgba(239,68,68,0.06)',  border: 'rgba(239,68,68,0.25)',   label: 'Urgent'    },
  high:   { icon: AlertTriangle, color: '#f97316', bg: 'rgba(249,115,22,0.06)',  border: 'rgba(249,115,22,0.25)',  label: 'Important' },
  normal: { icon: Info,          color: 'var(--primary)', bg: 'rgba(2,216,220,0.06)',  border: 'rgba(2,216,220,0.25)', label: 'Notice'    },
}
...
function AnnouncementCard({ ann }) {
  ...
  return (
    <Panel style={{ padding: 0, overflow: 'hidden', border: `1px solid ${open ? cfg.color : 'var(--border-subtle)'}`, background: open ? cfg.bg : 'var(--bg-card)', transition: 'var(--transition-normal)' }}>
      <button
      ...
      >
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', flexWrap: 'wrap' }}>
            <span style={{ fontSize: '0.9375rem', fontWeight: 800, color: 'var(--text-main)' }}>{ann.title}</span>
            <span style={{
      ...
      {open && (
        <div className="scale-in" style={{ padding: '0 1.25rem 1.5rem 4rem' }}>
          <p style={{ fontSize: '0.875rem', color: 'var(--text-muted)', lineHeight: 1.7, whiteSpace: 'pre-wrap' }}>{ann.body}</p>
        </div>
      )}
    </Panel>
  )
                ...
                {urgent.length > 0 && (
                   <div style={{ display: 'flex', alignItems: 'center', gap: '0.625rem', marginBottom: '0.5rem' }}>
                      <Bell size={16} style={{ color: 'var(--text-muted)' }} />
                       <p style={{ fontSize: '0.75rem', fontWeight: 900, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.08em' }}>All Updates</p>
                   </div>
                )}

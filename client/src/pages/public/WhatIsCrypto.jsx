...
export default function WhatIsCrypto() {
  ...
  return (
    <div style={{ background: BG, color: TEXT }}>
      ...
      <section style={{ 
        padding: '8rem 0 5rem', 
        background: `radial-gradient(circle at top right, rgba(2,216,220,0.05), transparent), radial-gradient(circle at bottom left, rgba(59,130,246,0.05), transparent)`,
        position: 'relative',
        overflow: 'hidden'
      }}>
        <div className="container" style={{ position: 'relative', zIndex: 1 }}>
          <div style={{ maxWidth: '800px', margin: '0 auto', textAlign: 'center' }}>
            ...
            <h1 style={{ 
              ...
              What is <span style={{ background: `linear-gradient(135deg, ${PRIMARY}, #3b82f6)`, WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text' }}>Cryptocurrency?</span>
            </h1>
      ...
      {/* Core Concepts */}
      <section style={{ padding: '5rem 0', background: SURFACE }}>
        <div className="container">
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '2rem' }}>
            {[
              ...
              <div key={i} style={{
                padding: '2.5rem', background: BG, border: `1px solid ${BORDER}`,
                ...
                <h3 style={{ fontSize: '1.5rem', fontWeight: 800, color: TEXT, marginBottom: '1rem' }}>{item.title}</h3>
                <p style={{ color: TEXT_SUB, lineHeight: 1.7 }}>{item.text}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      {/* The Paradigm Shift */}
      <section style={{ padding: '7rem 0', background: BG, borderTop: `1px solid ${BORDER}`, borderBottom: `1px solid ${BORDER}` }}>
        <div className="container">
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '5rem', alignItems: 'center' }} id="what-is-crypto-shift">
            <div>
              <h2 style={{ fontSize: '3rem', fontWeight: 900, marginBottom: '2rem', fontFamily: 'Outfit, sans-serif', color: TEXT }}>The Paradigm Shift</h2>
              <p style={{ fontSize: '1.125rem', color: TEXT_SUB, lineHeight: 1.8, marginBottom: '2.5rem' }}>
              ...
              <div style={{ display: 'grid', gap: '1.5rem' }}>
                {[
                  ...
                  <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                    <div style={{ width: 26, height: 26, borderRadius: '50%', background: 'rgba(2,216,220,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                      <ArrowRight size={14} color={PRIMARY} />
                    </div>
                    <span style={{ fontWeight: 700, color: TEXT }}>{point}</span>
                  </div>
                ))}
              </div>
            </div>
            <div style={{ position: 'relative' }}>
              <div style={{ 
                padding: '3rem', background: SURFACE, 
                border: `1px solid ${BORDER}`,
                borderRadius: 40, boxShadow: '0 32px 64px rgba(0,0,0,0.35)'
              }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '1.25rem', marginBottom: '2.5rem' }}>
                  ...
                  <div>
                    <h4 style={{ margin: 0, fontSize: '1.375rem', fontWeight: 900, color: TEXT }}>Digital Gold</h4>
                    <p style={{ margin: 0, fontSize: '0.875rem', color: 'var(--text-faint)', fontWeight: 600 }}>Decentralized Store of Value</p>
                  </div>
                </div>
                <div style={{ height: '220px', background: BG, borderRadius: 20, display: 'flex', alignItems: 'flex-end', padding: '1.5rem', gap: '0.75rem', border: `1px solid ${BORDER}` }}>
                  {[40, 60, 45, 70, 90, 85, 100].map((h, i) => (
                    <div key={i} style={{ flex: 1, height: `${h}%`, background: PRIMARY, borderRadius: '6px 6px 2px 2px', opacity: 0.2 + (i * 0.12) }} />
                  ))}
                </div>
      ...
      {/* CTA */}
      <section style={{ padding: '8rem 0', textAlign: 'center', background: SURFACE }}>
        <div className="container">
          <h2 style={{ fontSize: '2.5rem', fontWeight: 900, marginBottom: '1.5rem', fontFamily: 'Outfit, sans-serif' }}>Ready to start your journey?</h2>
          <p style={{ fontSize: '1.125rem', color: TEXT_SUB, marginBottom: '2.5rem' }}>Join thousands of traders on the most advanced platform.</p>
      ...
      <style>{`
        @media (max-width: 991px) {
          #what-is-crypto-shift { grid-template-columns: 1fr !important; gap: 3rem !important; }
        }
      `}</style>
    </div>
  )

  ...
  return (
    <div style={{ background: '#ffffff' }}>
      ...
      <section style={{ 
        ...
        background: `radial-gradient(circle at top right, rgba(0,180,166,0.06), transparent), radial-gradient(circle at bottom left, rgba(30,58,95,0.05), transparent)`,
        color: NAVY,
        position: 'relative',
        overflow: 'hidden'
      }}>
        <div style={{ position: 'absolute', top: '-10%', right: '-5%', width: '40%', height: '80%', background: `radial-gradient(circle, rgba(0,180,166,0.08) 0%, transparent 70%)`, borderRadius: '50%' }} />
        ...
        <div className="container" style={{ position: 'relative', zIndex: 1 }}>
          <div style={{ maxWidth: '800px' }}>
            <div style={{
              ...
              background: `rgba(0,180,166,0.06)`, border: `1px solid rgba(0,180,166,0.15)`,
            ...
            <p style={{ fontSize: '1.25rem', color: '#475569', lineHeight: 1.6, marginBottom: '3rem', maxWidth: '600px' }}>
            ...
            <div style={{ display: 'flex', gap: '1.5rem', flexWrap: 'wrap' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                <div style={{ display: 'flex' }}>
                  {[1, 2, 3].map(i => (
                    <div key={i} style={{ width: 32, height: 32, borderRadius: '50%', background: 'linear-gradient(135deg, #e6faf9, #e8f7fc)', border: '2px solid #fff', marginLeft: i === 1 ? 0 : -12 }} />
                  ))}
                </div>
                <span style={{ fontSize: '0.875rem', fontWeight: 700, color: '#64748b' }}>50k+ Active Members</span>
              </div>
            </div>
          </div>
        </div>
      ...
      <section style={{ padding: '7rem 0', background: '#f8fafc' }}>
        <div className="container">
          <div style={{ textAlign: 'center', marginBottom: '4rem' }}>
            ...
            <h2 style={{ fontFamily: 'Outfit, sans-serif', fontSize: 'clamp(2rem, 4vw, 2.75rem)', fontWeight: 900, color: NAVY, lineHeight: 1.2, letterSpacing: '-0.02em' }}>
              The Four Pillars of{' '}
              <span style={{ background: `linear-gradient(135deg, ${PRIMARY}, ${NAVY})`, WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text' }}>BitLance</span>
            </h2>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '2rem' }}>
            {[
              ...
              <div key={i} style={{
                ...
                onMouseEnter={e => { e.currentTarget.style.transform = 'translateY(-4px)'; e.currentTarget.style.boxShadow = `0 16px 40px rgba(0,0,0,0.06)` }}
                onMouseLeave={e => { e.currentTarget.style.transform = 'none'; e.currentTarget.style.boxShadow = 'none' }}
              >
                <pillar.icon size={40} style={{ color: pillar.color, marginBottom: '1.5rem' }} />
                <h3 style={{ fontSize: '1.5rem', fontWeight: 800, marginBottom: '1rem', color: NAVY }}>{pillar.title}</h3>
                <p style={{ color: '#64748b', lineHeight: 1.7, margin: 0 }}>{pillar.text}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section style={{ padding: '7rem 0' }}>
        <div className="container">
          <div style={{ maxWidth: '900px', margin: '0 auto' }}>
            <h2 style={{ fontSize: '2.5rem', fontWeight: 900, textAlign: 'center', marginBottom: '4rem', fontFamily: 'Outfit, sans-serif', color: NAVY }}>How We Compare</h2>
            <div style={{ overflow: 'hidden', border: '1.5px solid #e2e8f0', borderRadius: 32 }}>
              <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left' }}>
                <thead style={{ background: '#f8fafc' }}>
                  <tr>
                    <th style={{ padding: '1.5rem 2rem', fontSize: '0.875rem', fontWeight: 900, textTransform: 'uppercase', color: '#94a3b8' }}>Feature</th>
                    <th style={{ padding: '1.5rem 2rem', fontSize: '1rem', fontWeight: 900, color: PRIMARY }}>BitLance</th>
                    <th style={{ padding: '1.5rem 2rem', fontSize: '0.875rem', fontWeight: 700, color: '#64748b' }}>Others</th>
                  </tr>
                </thead>
                <tbody>
                  {[
                    ...
                    <tr key={i} style={{ borderTop: '1px solid #f1f5f9' }}>
                      <td style={{ padding: '1.25rem 2rem', fontWeight: 700, color: NAVY }}>{row.f}</td>
                      <td style={{ padding: '1.25rem 2rem', color: PRIMARY, fontWeight: 800 }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                          <CheckCircle2 size={16} /> {row.g}
                        </div>
                      </td>
                      <td style={{ padding: '1.25rem 2rem', color: '#94a3b8', fontWeight: 600 }}>{row.o}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
